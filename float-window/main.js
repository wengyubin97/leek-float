...
let hotkeyStatus = { configured: 'alt+q', attempted: 'Alt+Q', active: null, registered: false };
...
function applyHotkey() {
  ...
  const configured = getHotkey();
  const accelerator = configured === 'ctrl+q' ? 'Ctrl+Q' : 'Alt+Q';
  const registered = globalShortcut.register(accelerator, toggleWindow);
  hotkeyStatus = {
    configured,
    attempted: accelerator,
    active: registered ? accelerator : null,
    registered,
  };
  if (!registered) {
    console.error(`全局快捷键 ${accelerator} 注册失败（可能被其他程序占用）`);
  }
  if (win && !win.isDestroyed() && win.webContents) {
    win.webContents.send('hotkey-status', hotkeyStatus);
  }
  if (!registered && win && !win.isDestroyed()) {
    dialog.showMessageBox(win, {
      type: 'warning',
      title: '快捷键冲突',
      message: `全局快捷键 ${accelerator} 注册失败，可能已被其他应用占用。`,
      detail: 'Windows 的公开热键接口不会提供具体占用程序名称。请在其他应用中释放该组合键后，使用右键菜单“重新注册快捷键”。',
    }).catch(() => {});
  }
  return hotkeyStatus;
}
...
function hotkeyStatusLabel() {
  if (!hotkeyStatus.registered) return `未注册（${hotkeyStatus.attempted} 可能被占用）`;
  return `${hotkeyStatus.active} 已注册`;
}
...
ipcMain.handle('hotkey-set', (_event, value) => {
  if (value !== 'alt+q' && value !== 'ctrl+q') return hotkeyStatus;
  setHotkey(value);
  return applyHotkey();
});
